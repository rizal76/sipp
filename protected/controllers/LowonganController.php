...
class LowonganController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['LowonganTahap']))
		{
			$lowonganTahapsMasuk = array();
				        ...
				        foreach ($_POST['LowonganTahap'] as $j=>$modelp) {
				            ...
				            if (isset($_POST['LowonganTahap'][$j])) {
				            	//inisialisasi
				             	...
				             	// 	if(strlen(trim(CUploadedFile::getInstance($model,'file_tugas'))) > 0) 
						            // { 
						            //     $sss=CUploadedFile::getInstance($model,'file_tugas'); 
						            //     $lowonganTahapsMasuk[$j]->file_tugas = $lowonganTahapsMasuk[$j]->id_lowongan.'-'.$lowonganTahapsMasuk[$j]->id_tahap.'.'.$sss->extensionName; 						              			               
						           	// 	$lowonganTahapsMasuk[$j]->save(false);// models have already been validated
                 ...
                 //   					 $sss->saveAs(Yii::app()->basePath .'/../file_tugas/' . $lowonganTahapsMasuk[$i++]->file_tugas); 
				         		...
				         		// if($j==1){
				           //  		throw new Exception("satu insert", 1);	
				           //  	} 
				           //  	if($j==2){
				           //  		throw new Exception("dua insert", 1);	
				           //  	}
				            }
				        }
		...
		if(isset($_POST['Lowongan'] ,  $_POST['Tahap']))
		{
			...
			if($model->save()) {
				//$tahaps =  $_POST['Tahap'];
				// Yii::app()->runController('lowonganTahap/createBatch',array(
				// 'idLowongan'=>$model->id,'tahaps'=>$tahaps,
				// ));
